...
$(function() {
	...
	if ($("#salesChart").length > 0) {
		...
		$("body").data("_dworker_sale_chart", salesChart)
		salesChartArea = salesChart;
	}
	...
	/*
	 * jVector Maps ------------ Create a world map with markers
	 */
...
function renderData(data){
	...
	$("#salesChartParent").empty().append("<canvas id='salesChart' height='180'></canvas>");
	var chartCanvas = $("#salesChart").get(0);
	var salesChartCanvas = $("#salesChart").get(0).getContext("2d");
	salesChartCanvas.clearRect(0,0,chartCanvas.width,chartCanvas.height);
	// This will get the first returned node in the jQuery collection.
	var salesChart = new Chart(salesChartCanvas);
